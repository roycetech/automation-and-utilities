...
class CodeSchoolDownloader
  ...
  def login(username, password)
    ...
    puts 'Loaded, typing credentials...'
    ...
    puts('Placed credentials, clicking submit...')
  ...
  def deal_with_courses
    ...
    ListProvider.video_course_urls.each do |url|
  ...
  def download(url, dir_name, passed_in_filename = nil)
    ...
    sub_dir_name = dir_name + '/' + page.css('h1').text.gsub('Screencast', '')
                   .strip.gsub(/\W/, ' ')
                   .gsub(/\s+/, ' ').gsub(/\s/, '-')
    ...
    links.each do |course|
      if counter >= START_INDEX && counter <= END_INDEX # begin skip
        begin
          ...
          src_selector = 'div#level-video-player video'
          url = video_page.css(src_selector).attribute('src').value

          puts 'URL retrieved'
          puts 'Closing video...'

          modal_close = @browser.links(:class, 'modal-close')[3]
          modal_close.when_present.fire_event('click')

          idx_str = format('%02d', counter)
          name = passed_in_filename || "#{idx_str} - #{filenames[counter - 1]}"

          filename = "#{sub_dir_name}/#{name}.mp4"

          max_retry = 5

          if File.exist?(filename) && File.size?(filename)
            counter += 1
            next
          end
  ...
  def download_screencasts(url, dir_name, passed_in_filename = nil)
    ...
    h1_text = page.css('h1 span:last').text.tr(%r{\/}, '-')
    sub_dir_name = dir_name + '/' + h1_text
    ...
    begin
      ...
      video_url = video_page.css('div#code-school-screencasts video')
                            .attribute('src').value

      puts "VIDEO URL retrieved: #{video_url}"
      puts 'Closing video...'
      @browser.back
      name = page.css('h1').text.gsub('Screencast', '').strip
                 .gsub(/\W/, ' ')
                 .gsub(/\s+/, ' ')
                 .gsub(/\s/, '-')
...
# It generates links for the screen casts.
class LinkGenerator
  def self.screencast_urls
    ...
    screencast_page.css(screencast_selector).each do |element|
      ...
      screencast_urls << 'https://www.codeschool.com' + href
    end
    screencast_urls
  end
  ...
  def self.course_urls
    ...
    course_page.css(course_selector).each do |element|
      href = element.attributes['href'].value
      course_urls << "https://www.codeschool.com#{href}/videos"
    end
    course_urls
  end
end
